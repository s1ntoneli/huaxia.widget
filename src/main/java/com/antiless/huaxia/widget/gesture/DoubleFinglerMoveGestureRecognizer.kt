...
class DoubleFinglerMoveGestureRecognizer(gesturePointersUtility: GesturePointersUtility) : BaseGestureRecognizer<DoubleFingerMoveGesture>(gesturePointersUtility) {
    override fun tryCreateGestures(event: MotionEvent) {
        ...
        for (it in 0 until event.pointerCount) {
            ...
            gestures.add(DoubleFingerMoveGesture(gesturePointersUtility, event, pointerId))
        }
    }
