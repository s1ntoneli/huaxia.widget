...
class PinchGestureRecognizer(gesturePointersUtility: GesturePointersUtility) : BaseGestureRecognizer<PinchGesture>(gesturePointersUtility) {
    override fun tryCreateGestures(event: MotionEvent) {
        ...
        for (it in 0 until event.pointerCount) {
            ...
            gestures.add(PinchGesture(gesturePointersUtility, event, pointerId))
        }
    }
